...
void requestRestart(HttpConn *conn)
{
    HttpQueue   *q;
    q = conn->writeq;
    ...
    httpWrite(q, "<!DOCTYPE HTML><html>"
        "<head>"
            "<meta http-equiv=\"content-type\" content=\"text/html; charset=utf-8\">"
            "<title>Restart</title>"
        "</head>"
        "<body>"
        "<p>Restart reqested</p>"
        "</body></html>"
    );
    restart = 1;
    httpFinalize(conn);
...
void requestStop(HttpConn *conn)
{
    HttpQueue   *q;
    q = conn->writeq;
    httpSetStatus(conn, 200);
...
static int createEndpoints(int argc, char **argv)
{
    ...
    httpDefineAction("/action/restart", requestRestart);
    httpDefineAction("/action/stop", requestStop);
    return 0;
}
